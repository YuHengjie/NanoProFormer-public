...
# %%
loaded_threshold_binary = np.load('../modal_fusion/threshold_binary.npy', allow_pickle=True)
threshold_tensor = torch.tensor(loaded_threshold_binary, device=device)
threshold_tensor
...
classifier.load_state_dict(torch.load("../modal_fusion/best_model_binary.pt"))
classifier.eval() 
...
# %%
test_df['Probs'] = metrics['Probs']
test_df

# %% 
auc_score = roc_auc_score(test_df['Affinity_Category'].values, test_df['Probs'].values)
auc_score

# %%
test_df.to_csv('./train_result_binary/test_df_on_normal.csv')
