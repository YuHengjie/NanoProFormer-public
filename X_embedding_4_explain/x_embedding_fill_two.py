...
# %%
df = pd.read_csv("../data/df_combine_fill.csv",keep_default_na=False, na_values=[''])
df
...
# %%
feature_columns = df_x.columns[1:30]
feature_columns
...
for col1, col2 in feature_pairs:
    ...
    file_name = f"x_embeddings_{cleaned_col1}_{cleaned_col2}.npy"
    ...
    cleaned_col1 = ''.join(c if c.isalpha() else '_' for c in col1)
    cleaned_col2 = ''.join(c if c.isalpha() else '_' for c in col2)
    np.save(f"./fill_two/x_embeddings_{cleaned_col1}_{cleaned_col2}.npy", embeddings)
