...
# %%
df = pd.read_csv("../data/df_combine_non_fill.csv",keep_default_na=False, na_values=[''])
df
...
for col1, col2 in feature_pairs:
    ...
    cleaned_col1 = ''.join(c if c.isalpha() else '_' for c in col1)
    cleaned_col2 = ''.join(c if c.isalpha() else '_' for c in col2)
    np.save(f"./non_fill_two/x_embeddings_{cleaned_col1}_{cleaned_col2}.npy", embeddings)
